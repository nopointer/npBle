...
class Peripheral extends BluetoothGattCallback {
    ...
    public Peripheral(BleDevice device) {
        this.scanRecord = device.getScanBytes();
        this.rssi = device.getRssi();
        this.name = device.getName();
        this.mac = device.getMac();
        this.device = BluetoothAdapter.getDefaultAdapter().getRemoteDevice(device.getMac());
    ...
    public void connect(Context context) {
        ...
        synchronized (this.mStateLock) {
            if (this.mConnState == CONN_STATE_IDLE) {
                NpBleLog.d("connect " + this.getDeviceName() + " -- "
                        + this.getMacAddress());
    ...
    public void disconnect() {
        ...
        NpBleLog.d("disconnect " + this.getDeviceName() + " -- "
                + this.getMacAddress());
...
//        ycBleLog.d("processing : " + this.processing);
...
//        ycBleLog.d("processCommand : " + command.toString());
...
//        ycBleLog.d("commandSuccess");
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        NpBleLog.d("onConnectionStateChange  status :" + status + " state : " + newState);
        ...
        if (newState == BluetoothGatt.STATE_CONNECTED) {
            ...
            if (this.gatt == null || !this.gatt.discoverServices()) {
                NpBleLog.d("remote service discovery has been stopped status = "
                        + newState);
...
//        Log.e("debug==change>", BleUtil.byte2HexStr(characteristic.getValue()));
...
//        Log.e("debug== read--->UUID>", characteristic.getUuid().toString());
//        Log.e("debug== read>", BleUtil.byte2HexStr(characteristic.getValue()));
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt,
        ...
        this.cancelCommandTimeoutTask();
//        Log.e("debug==write--->UUID>", characteristic.getUuid().toString());
//        Log.e("debug==write>", BleUtil.byte2HexStr(characteristic.getValue()));
        if (status == BluetoothGatt.GATT_SUCCESS) {
...
//        ycBleLog.d("onCharacteristicWrite newStatus : " + status);
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            NpBleLog.d("Service discovery failed");
            this.disconnect();
        }
    }
