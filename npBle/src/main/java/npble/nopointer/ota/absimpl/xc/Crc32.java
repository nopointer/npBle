...
public class Crc32 {
    ...
    public static final long CRC32_XOR_VAL = 0xFFFFFFFFL;
    private long[] l_crc32_table ={
    ...
    static int  bitrev(long input, long bw) {
        int i;
        int var=0;
        for (i = 0; i < bw; i++) {
            if ((input & 0x01) == 1) {
   ...
   public long  crc32_calc(long crc, byte[] input, int len) {
       ...
       long crc32 = crc;
        for (int i = 0; i < len; i++) {
            ...
            crc32 = l_crc32_table[(int) a] ^ (crc32 >> 8);
        }

        return (crc32 ^ (CRC32_XOR_VAL));
    }
