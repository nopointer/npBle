 ...
 */
public class XSwipeMenuLayout extends FrameLayout implements Controller {
    ...
    public XSwipeMenuLayout(Context context) {
        this(context, null);
    }

    public XSwipeMenuLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XSwipeMenuLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        setClickable(true);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.XSwipeMenuLayout);
        mLeftViewId = typedArray.getResourceId(R.styleable.XSwipeMenuLayout_xLeftViewId, mLeftViewId);
        mContentViewId = typedArray.getResourceId(R.styleable.XSwipeMenuLayout_xContentViewId, mContentViewId);
        mRightViewId = typedArray.getResourceId(R.styleable.XSwipeMenuLayout_xRightViewId, mRightViewId);
        typedArray.recycle();
    ...
    float distanceInfluenceForSnapDuration(float f) {
        f -= 0.5f; // center the values-zh about 0.
        f *= 0.3f * Math.PI / 2.0f;
