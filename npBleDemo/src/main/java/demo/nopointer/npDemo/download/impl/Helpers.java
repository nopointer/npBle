...
public class Helpers {
	...
	private static String getPathForFileUri(String url, String hint,
		...
		if (path.endsWith("/")) {
			...
			path = generateFilePath(basePath,hint, url, contentDisposition,
	...
	private static String generateFilePath(String basePath,String hint, String url,
			String contentDisposition, String contentLocation, String mimeType,
			int destination, long contentLength) throws GenerateSaveFileError {
	...
	private static String chooseFullPath(Context context, String url,
		...
		return generateFilePath(base.getPath(),hint,url, contentDisposition,contentLocation, mimeType, destination, contentLength);
	}
	...
	private static File locateDestinationDirectory(Context context,
			String mimeType, int destination, long contentLength)
			throws GenerateSaveFileError {
		return getExternalDestination(context,contentLength,destination);
	}

	private static File getExternalDestination(Context context,long contentLength,int destination)throws GenerateSaveFileError {
		//放system/cache目录
		File root = null;
		if(destination == Downloads.DESTINATION_SYSTEMCACHE_PARTITION){
			root = context.getFilesDir();
//			root = new File("/data/local/tmp/");
		}else {
			if (!isExternalMediaMounted()) {
		 ...
		 root = new File(Environment.getExternalStorageDirectory().getPath() + Constants.DEFAULT_DL_SUBDIR) ;
		...
		if (!root.isDirectory() && !root.mkdir()) {
			...
			throw new GenerateSaveFileError(Downloads.STATUS_FILE_ERROR,
					"unable to create external downloads directory " + root.getPath());
		}
		return root;
	}
