 ...
 * 支持类型 : 下滑线，三角形，全背景，图片
 * <p>
 * https://github.com/RuffianZhong/YCIndicator
 *
 * @author ZhongDaFeng
 */
public class YCIndicator extends LinearLayout {
    ...
    public YCIndicator(Context context) {
        this(context, null);
    }

    public YCIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
        setGravity(Gravity.CENTER_VERTICAL);
        // 获得自定义属性
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.YCIndicator);
        mTabVisibleCount = a.getInt(R.styleable.YCIndicator_indicator_visible_count, D_TAB_COUNT);
        mTextColorNormal = a.getColor(R.styleable.YCIndicator_text_color_normal, D_TEXT_COLOR_NORMAL);
        mTextColorHighlight = a.getColor(R.styleable.YCIndicator_text_color_selected, D_TEXT_COLOR_HIGHLIGHT);
        mTextSize = a.getDimensionPixelSize(R.styleable.YCIndicator_text_size, dip2px(context, 16));
        mIndicatorColor = a.getColor(R.styleable.YCIndicator_indicator_color, D_INDICATOR_COLOR);
        mIndicatorStyle = a.getInt(R.styleable.YCIndicator_indicator_style, STYLE_LINE);
        mStyleLinePadding = a.getDimensionPixelSize(R.styleable.YCIndicator_style_line_padding, 0);
        mIndicatorHeight = a.getDimensionPixelSize(R.styleable.YCIndicator_indicator_height, -1);
        Drawable drawable = a.getDrawable(R.styleable.YCIndicator_style_bitmap_src);
