...
package demo.nopointer.npDemo.download.impl;
...
public class DownloadManager {
	...
	public final static String COLUMN_LOCAL_DATA = "_data";
	 ...
	 * url is not found 
	 */
	public final static int ERROR_FILE_URL_NOT_FOUND = 1010;
	...
	public final static String ACTION_DOWNLOAD_VALUE = "android.intent.action.DOWNLOAD_VALUE";
	...
	private static final String[] COLUMNS = new String[] { COLUMN_ID,
			...
			COLUMN_TOTAL_SIZE_BYTES, COLUMN_LOCAL_URI, COLUMN_LOCAL_DATA,COLUMN_STATUS,
	...
	public static class Request {
		...
		ContentValues toContentValues(String packageName) {
			...
			if (mDestinationUri != null) {
				values.put(Downloads.COLUMN_DESTINATION,(this.mUseSystemCache) ? Downloads.DESTINATION_SYSTEMCACHE_PARTITION : Downloads.DESTINATION_FILE_URI);
...
//				values.put(Downloads.COLUMN_DESTINATION,(this.mUseSystemCache) ? Downloads.DESTINATION_SYSTEMCACHE_PARTITION : Downloads.DESTINATION_CACHE_PARTITION_PURGEABLE);
				//直接修改成下载到/cache目录下
				values.put(Downloads.COLUMN_DESTINATION, Downloads.DESTINATION_SYSTEMCACHE_PARTITION);
			}
	 ...
	 * @return
	 */
	public String getFileNameByDownloadId(long downloadId){
		String fileName = "";
		Query query = new Query().setFilterById(downloadId);
		Cursor c = null;
		try {
   ...
   */
    ...
    public String getDownloadPath(long downloadId){
        ...
        try {
            c = query(query);
            if (c != null && c.moveToFirst()) {
                resultString = c.getString(c.getColumnIndexOrThrow(DownloadManager.COLUMN_LOCAL_DATA));
            }
        } finally {
            if (c != null) {
                c.close();
	...
	private static class CursorTranslator extends CursorWrapper {
		...
		private String translateString(String column) {
			...
			if(column.equals(COLUMN_LOCAL_DATA)){
				return getUnderlyingString(Downloads._DATA);
			}
